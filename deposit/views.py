...
def deposit_list_for_current_user(request):
    ...
    if request.user.is_authenticated:
        ...
        deposits = Deposit.objects.filter(gambler=gambler_profile)
        print(deposits)
        return render(request, 'deposit/deposit_list_for_user.html', {'deposits': deposits})
    else:
        return render(request, 'deposit/not_logged_in.html')
...
def deposit_moncash(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'deposit/payment_error.html', {'error_message': 'Invalid deposit amount.'})
        except ValueError as e:
            return render(request, 'deposit/payment_error.html', {'error_message': str(e)})
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                return render(request, 'deposit/payment_error.html', {'error_message': error_message})
    ...
    if request.method == 'POST':
        try:
            ...
            with transaction.atomic():
                ...
                balance = Balance.objects.filter(gambler=gambler_profile).first()
                if balance:
                    balance.balance_globale += payment_details.get('cost')
                    balance.save()
                else:
                    # Create a new balance entry if it doesn't exist
                    balance = Balance.objects.create(gambler=gambler_profile, balance_globale=payment_details.get('cost'))
