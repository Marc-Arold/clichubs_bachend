...
def withdrawal_list(request):
    ...
    return render(request, 'withdrawals/withdrawal_list.html', {'withdrawals': withdrawals})


def withdrawal_list_for_current_user(request):
    if request.user.is_authenticated:
        current_user_id = request.user.id
        withdrawals = Withdrawal.objects.filter(gambler=current_user_id)
...
def withdraw_moncash(request):
    ...
    try:
            ...
            return render(request, 'withdrawals/withdrawalerror.html', {'error_message': 'Invalid withdraw amount.'})
    except ValueError as e:
            return render(request, 'withdrawals/withdrawalerror.html', {'error_message': str(e)})
        ...
        if response.status_code == 200:
            ...
            if 'transfer' in data and 'transaction_id' in data['transfer']:
                ...
                # Return a JSON response with the success message
                    balance = Balance.objects.filter(gambler = gambler_profile)
